    ...
    playPodcast() {
        if(!this.statusInterval)
        {
            this.statusInterval = setInterval(async () => {
                let strMpdState = await this.getMpdStatus();
                let mpdState = mpd.parseKeyValueMessage(strMpdState);
                //log progress as percentage 
                let progress = parseInt(mpdState.elapsed / mpdState.duration,10);
                //elapsed time as mm:ss making sure to pad mm and ss seperately with 0 if needed
                let elapsed = `${Math.floor(mpdState.elapsed / 60).toString().padStart(2, '0')}:${Math.floor(mpdState.elapsed % 60).toString().padStart(2, '0')}`;

                
                //duration as mm:ss
                let duration = `${Math.floor(mpdState.duration / 60)}:${Math.floor(mpdState.duration % 60)}`;

                this.statusString = `${progress}% ${elapsed}/${duration}`;
